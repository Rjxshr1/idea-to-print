...
| 已适配的图片转3D | 公共Hunyuan3D-2.1服务、`gradio_client` | `requirements.txt`；实测匿名调用，需联网；不保证一直无需登录 | 推理发生在服务端，不占本机推理显存 |
...
对原始百万面模型，应先保留原件，评估简化对细节的影响，再降低面数。避免同时运行多份大模型操作。无法从STL文件大小单独精确推算所需RAM。
